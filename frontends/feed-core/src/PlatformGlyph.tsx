...
// Each glyph carries its own brand color (a token, never a literal) so it reads correctly anywhere
// it is dropped in, not only where a particular stylesheet is in scope. Platform color stays one of
// the two sanctioned accents (the rail and this glyph). Size and spacing come from the caller.
...
export default function PlatformGlyph({ platform, className }: { platform: Platform; className?: string }) {
  const style: CSSProperties = { color: COLOR[platform] };
  switch (platform) {
    case 'twitch':
      return (
        <svg className={className} style={style} data-platform="twitch" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
    ...
    case 'x':
      return (
        <svg className={className} style={style} data-platform="x" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
