import { Field, Input, Segmented, Select, Textarea } from '@virta/ui-kit';
...
// Renders a config form from a plugin's JSON Schema using the same Field primitive as Settings —
// a plugin never writes its own form. Each change merges into the value object and is handed
// back; persistence (per-profile, under the plugin id) is the caller's job.
          ...
          <Field key={f.key} label={label} hint={f.hint}>
            {f.kind === 'toggle' ? (
              ...
              <Input
                ...
                onChange={(e) =>
                  set(f.key, f.kind === 'number' ? Number(e.currentTarget.value) : e.currentTarget.value)
                }
              />
            )}
          </Field>
